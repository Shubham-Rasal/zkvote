...
function Home() {
  const [ethereumAddress, setEthereumAddress] = useState('0x0b89c86f69f560e865a529adda7e8ee269bf7fdd')
          ...
          <div className="flex justify-between h-16">
            <div className="flex">
              <div className="flex-shrink-0 flex items-center">
                <h1 className="text-xl font-bold text-gray-800">Voting Dashboard</h1>
              </div>
            </div>
        ...
        <div className="px-4 py-6 sm:px-0">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {elections.map((election) => (
              <div key={election.id} className="bg-white overflow-hidden shadow rounded-lg">
                <div className="px-4 py-5 sm:p-6">
                  <h3 className="text-lg leading-6 font-medium text-gray-900">{election.name}</h3>
                  <div className="mt-5">
                    <p className="text-sm text-gray-500">
                      <span className="font-medium text-gray-700">Votes:</span> {election.votes.toLocaleString()}
                    </p>
                    <p className="mt-1 text-sm text-gray-500">
                      <span className="font-medium text-gray-700">Winner:</span> {election.winner}
                    </p>
                    <p className="mt-1 text-sm text-gray-500">
                      <span className="font-medium text-gray-700">Date:</span> {election.date}
                    </p>
                  </div>
                </div>
              </div>
